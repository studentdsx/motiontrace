...
服务端同时提供 Web 管理后台，用于查询轨迹点和排查上传提交记录。后台有独立登录页，默认账号为 `admin`，默认密码为 `Admin@1357`。
...
管理后台访问 `https://motiontrace.631581.xyz/admin`，未登录时会跳转登录页；输入默认账号 `admin` 和默认密码 `Admin@1357` 后可按用户、行程 ID、日期和时间范围查询 GPS 点位，并支持列表/地图视图切换和 CSV 导出。
