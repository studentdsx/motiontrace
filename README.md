...
- 可选云同步：用户名注册/登录、邮箱留作找回密码信息、修改密码、上传本机数据、从云端恢复、记录中实时回传轨迹点。
- Cloudflare Worker 提供账号 API、轨迹快照同步、结构化轨迹点入库、轨迹管理后台、用户管理后台和 CSV 导出。
...
- 云同步当前上传轨迹、行程、打卡坐标、地名和文字信息；开启实时同步后，记录过程中会持续把新增 GPS 点位写入 `track_points` 表，便于后台按用户、日期和行程查询。跨天行程按开始记录日期归档。
- 打卡照片原图暂不上传，仍保存在手机本机。
